...
class OccupationalDetail extends StatelessWidget {

  final String type;
  OccupationalDetail({required this.type});

  @override
  Widget build(BuildContext context) {
    return MaterialApp(
      home: OccupationalDetailStateful(type),
...
class OccupationalDetailStateful extends StatefulWidget {
  ...
  final String type;
  OccupationalDetailStateful(this.type);
...
class OccupationalDetailScreen  extends State<OccupationalDetailStateful>{
  ...
  initView() async {
    ...
    occupationController.text = utils().replaceNull(prefs.getString(SharedPrefs.occupation).toString().split(",")[0]);
    occupdetailsController.text = utils().replaceNull(prefs.getString(SharedPrefs.occupationDetail).toString());
    annualincomeController.text =   utils().replaceNull(prefs.getString(SharedPrefs.annualIncome).toString());
    employmenttypeController.text =  utils().replaceNull(prefs.getString(SharedPrefs.employmentType).toString());
    officeaddressController.text =   utils().replaceNull(prefs.getString(SharedPrefs.officeAddress).toString());

    occupation_value = prefs.getString(SharedPrefs.occupation).toString().split(",")[1];
    occu_details = prefs.getString(SharedPrefs.occupationDetail).toString();
    annual_inconme = prefs.getString(SharedPrefs.annualIncome).toString();
    employment_value = prefs.getString(SharedPrefs.employmentType).toString();
  ...
  Widget build(BuildContext context) {
    ...
    SizedBox(height: 20,),
      CustomDropdown(icondata: Icons.person , controller: annualincomeController , labelText: TranslationService.translate("annual_income_prefs"), onButtonPressed: () async {
        ...
        final value = await SingleSelectDialog().showBottomSheetAnnualincome(context , list_annual_income);
        annualincomeController.text = value.label;
   ...
   ConnectivityResult result = await Connectivity().checkConnectivity();
    ...
    DialogClass().showDialog2(context, "No Internet", "Sorry Internet is not available", "OK");
      ...
      SharedPreferences prefs = await SharedPreferences.getInstance();
      if (prefs.getString(SharedPrefs.occupation) == null) {
        EasyLoading.show(status: 'Please wait...');
        ...
        if (_response.body["data"]["affectedRows"] == 1) {
          ...
          await prefs.setString(
              SharedPrefs.annualIncome, annualincomeController.text.toString());
          await prefs.setString(SharedPrefs.employmentType,
              employmenttypeController.text.toString());
          await prefs.setString(SharedPrefs.officeAddress,
              officeaddressController.text.toString());
          await prefs.setString(SharedPrefs.occupation_id,
              _response
          ...
          if (widget.type == "insert") {
            navService.pushNamed("/horoscope_details");
          } else {
            navService.goBack();
          }
        } else {
        ...
        final _response = await Provider.of<ApiService>(
            ...
            .postOccupationUpdate(
            {
              ...
              "employment_type": employmenttypeController.text.toString(),
              "office_address": officeaddressController.text.toString(),
              "Id": prefs.getString(SharedPrefs.occupation_id)
            }
        );
